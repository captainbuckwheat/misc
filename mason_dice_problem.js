...
var run_over_12 = function(n) { //where n is the number of trials (the higher is n the more accurate probability will be)
	var i, count;
	count = []; //histogram
	...
	for (i = 0; i < count.length; i++) count[i] /= n; // which is 18-13+1=6, we could just say 6 instead of count.length
	return count; 
}

var probability_of = function(a, n) { //a is a sum from 13 to 18 inclusive; n is number of trials - should be high for more accurate probab.
	var count, sum, i; 
	count = 0; 
	for (i = 0; i < n; i++) {
		sum = 0;
...
var test = function() {
	var i, got, want, cases, precision;
	precision = 0.02;
	cases = [
		[13, 10000], //first is i in count[i] from run_over_12(); second is n for number of trials;
		[14, 20000],
		[15, 30000],
		[16, 100000],
		[17, 60000],
	...
	for (i = 0; i < cases.length; i++) {
		...
		if (got - want > precision || want - got > precision ) {
			console.log("run_over_12(" + cases[i][1]+ ")[" + (cases[i][0]-13)+"]: got " + got + "; want " + want)
